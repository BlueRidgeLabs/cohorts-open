...
class Admin::MailchimpUpdatesController < Admin::BaseController
  ...
  def index
    @mailchimp_updates = MailchimpUpdate.order('fired_at DESC').page params[:page]
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_mailchimp_updates_url }
      format.json { head :no_content }
    end
  end
