...
class Submission < ActiveRecord::Base
  ...
  before_create :find_form, if: -> { form.nil? }
  ...
  def form_email
    JSON.parse(field_structure)['Fields'].each do |field|
      return field_value(field['ID']) if ['email', 'email address'].include?(field['Title'].downcase)
    end
    nil
  end

  def form_phone_number
    JSON.parse(field_structure)['Fields'].each do |field|
      return field_value(field['ID']) if ['phone', 'phone number'].include?(field['Title'].downcase)
    end
    nil
  end

  def find_person
    if form_email || form_phone_number
      self.person = Person.find_by(
        'email_address = ? or phone_number = ?',
        form_email,
        PhonyRails.normalize_number(form_phone_number, country_code: 'US')
      )
    end
  end
  ...
  def find_form_type
    self.form_type = form_type_field.downcase
  rescue
  ...
  def find_wufoo_entry
    self.field_structure = { 'Fields' => form.fields }.to_json
    self.form_structure = form.details.to_json
    self.raw_content = form.wufoo_entry(entry_id).to_json
  end
  ...
  private
    ...
    def find_form
      Form.update_forms
      self.form = Form.find_by(hash_id: form_hash)
    end
